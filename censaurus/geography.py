...
from censaurus.constants import LAYER_NAME_MAP, LAYER_NAME_MAP_REVERSE, FEATURE_ATTRIBUTE_MAP
    ...
    def _build_geography_params(self, areas: AreaCollection, within: Union[Area, List[Area]], target: str, target_layer_name: Union[str, List[str]], return_geometry: bool, area_threshold: float):        
        if isinstance(within, Area):
            ...
            for geography in geographies:
                ...
                if highest_parent_geography is None or int(min(parent_geography, key=lambda g : int(g.level)).level) < highest_parent_geography_level:
                    child_geography = geography
                    highest_parent_geography = min(parent_geography, key=lambda g : int(g.level))
                    highest_parent_geography_level = int(min(parent_geography, key=lambda g : int(g.level)).level)

            if highest_parent_geography:
                if highest_parent_geography != child_geography:
                    if highest_parent_geography.name == target:
                        highest_parent_layer_name = target_layer_name
                    elif highest_parent_geography.name in LAYER_NAME_MAP_REVERSE:
                        highest_parent_layer_name = LAYER_NAME_MAP_REVERSE[highest_parent_geography.name]
                    else:
                        highest_parent_layer_name = highest_parent_geography.name

                    parent_layer = areas.get_layer(highest_parent_layer_name)

                    geographic_columns = [FEATURE_ATTRIBUTE_MAP[c] for c in parent_layer.geographic_fields]
                    parents = areas.get_features_within(within=within, layer_name=parent_layer.name, area_threshold=area_threshold, return_attributes=True, return_geometry=False)[geographic_columns].to_dict(orient='records')
                    params_list = []

                    for p in parents:
                        feature_attributes = {k: v for k, v in p.items() if v is not None}
                        geography_params = child_geography._build_broadest_params(feature_attributes=feature_attributes)
                        params_list.append(geography_params)
                else:
                    params_list = [child_geography._build_broadest_params(feature_attributes={})]

                if return_geometry:
                    features_within = areas.get_features_within(within=within, layer_name=target_layer_name, area_threshold=area_threshold, return_attributes=False, return_geometry=True)
                    return child_geography, params_list, features_within
                else:
                    return child_geography, params_list, None
        ...
        features_within = areas.get_features_within(within=within, layer_name=target_layer_name, area_threshold=area_threshold, return_attributes=True, return_geometry=return_geometry)
